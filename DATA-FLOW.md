...
3. **User reacts** — clicks approve, edits, rejects, ignores, mutes, does the task themselves. *Inaction is a reaction too.*
...
- **One reaction can update multiple memories.** A user editing a draft says something about *what was selected* (probably the right task), *how it was done* (the artifact needed correction), and *when it surfaced* (timing was probably fine). The framework lets one event contribute to all three memories.
